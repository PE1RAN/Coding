...
public:
    ...
    virtual void updatePriorities() {
        ...
        Node* iter = root;
        toDeque(iter, deck);
        PairingPQ helper { deck.cbegin(), deck.cend(), this->compare };
        *this = helper;
...
private:
    ...
    void meld(Node*& subj, Node* obj) {
        if (obj == nullptr)
            return;
        if (this->compare(subj->elt, obj->elt)) {
            //obj->prev = subj->prev;
            subj->prev = obj;
            subj->sibling = obj->child;
            if (subj->sibling != nullptr)
                subj->sibling->prev = subj;
            obj->child = subj;
            subj = obj;
        }
        else {
            obj->prev = subj;
            /*subj->sibling = obj->sibling;
            if (subj->sibling != nullptr)
                subj->sibling->prev = subj;
            obj->sibling = subj->child;
            if (obj->sibling != nullptr)
                obj->sibling->prev = obj;
            subj->child = obj;*/
            obj->sibling = subj->child;
            if (obj->sibling != nullptr)
                obj->sibling->prev = obj;
    ...
    void freeChilds(Node* node) {
        if (node != nullptr) {
            freeChilds(node->child);
            freeChilds(node->sibling);
            delete node;
        }
    }
    ...
    Node* copy(const Node* node) {
        if (node != nullptr) {
            Node* baby = new Node(node->elt);
            if ((baby->child = copy(node->child)) != nullptr)
                baby->child->prev = baby;
            if ((baby->sibling = copy(node->sibling)) != nullptr)
                baby->sibling->prev = baby;
            return baby;
        }
        return nullptr;
    }
    void toDeque(Node* iter, std::deque<TYPE>& deck) {
        if (iter != nullptr) {
            deck.push_back(iter->elt);
            toDeque(iter->child, deck);
            toDeque(iter->sibling, deck);
        }
    }
    Node* unionChilds(Node* prince) {
        if (prince->sibling == nullptr) {
            prince->prev = nullptr;
            return prince;
        }
          
        std::deque<Node*> duke;
        while (prince != nullptr) {
            duke.push_back(prince);
            prince->prev->sibling = nullptr;
            prince->prev = nullptr;
            prince = prince->sibling;
        }
        while (duke.size() > 1) {
            meld(duke[0], duke[1]);
            duke.push_back(duke[0]);
            duke.pop_front();
            duke.pop_front();
        }
        return duke.back();
