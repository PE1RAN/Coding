...
public:
  ...
  Map(const Map& other) : volumn(other.volumn), ktree(other.ktree) {}
  ...
  Value_type& operator[](const Key_type& k) {
    Value_type v = Value_type();
    Pair_type p(k, v);
    std::pair<Iterator, bool> pr = insert(p);
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    ...
    if (it != ktree.end()) {
      std::pair<Iterator, bool> p(it, false);
      return p;
    }
    else {
      std::pair<Iterator, bool> p(ktree.insert(val), true);
      return p;
    }
  };
...
private:
  ...
  size_t volumn;
  BinarySearchTree<Pair_type, PairComp> ktree;
};
