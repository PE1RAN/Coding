...
transformed parameters {
  ...
  real pred[n];
  for (i in 1:N) {
    regr[i] = intercept + Pclass[1]*class1_train[i] + Pclass[2]*class2_train[i] + Sex_Male*sex_train[i] + Age*age_train[i];// + beta[6]*family_train[i]);// + beta[6]*old_train[i]);// + beta[6]*sib_train[i] + beta[7]*pch_train[i]);
    }
  for (i in 1:n) {
    pred[i] = intercept + Pclass[1]*class1_test[i] + Pclass[2]*class2_test[i] + Sex_Male*sex_test[i] + Age*age_test[i];// + beta[6]*family_test[i]);// + beta[6]*old_test[i]);// + beta[6]*sib_test[i] + beta[7]*pch_test[i]);
    //pred[i] = inv_logit(pred_temp[i]); 
  }
}
...
model {
  ...
  for (i in 1:N) {
    //regr_temp[i] ~ normal(train, sigma);
    vive_train[i] ~ normal(regr,sigma);
    //if (regr[i] > 0.5) vive_train ~ bernoulli(1);
    //else vive_train ~ bernoulli(0);
  }
